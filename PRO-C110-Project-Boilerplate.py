...
while True:
	...
	if status:
		...
		resised_frame = cv2.resize(frame, (224,224))

		# Expanda a dimensão do array junto com o eixo 0
		resised_frame = np.expand_dims(resised_frame, axis = 0)

		# Normalize para facilitar o processamento
		resised_frame = resised_frame/255

		# Obtenha previsões do modelo
		predictions = mymodel.predict(resised_frame)
